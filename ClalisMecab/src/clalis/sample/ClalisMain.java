...
//  概要      : Clalis 形態素解析 ClalisMecab サンプル
//
...
public class ClalisMain {
	...
	public static void main(String[] args) {
		try{
		 	...
		 	//結果の取得
		 	String jsonText = inputStreemToString(post("http://liplis.mine.nu/Clalis/v30/Post/Json/clalisMecab.aspx", nameValuePair).getEntity().getContent());

		 	//API結果受け取り用クラス
		 	ResMecabResult result =  new Gson().fromJson(jsonText, ResMecabResult.class);

		 	//ResMecabResultクラスの「resWordList」プロパティに結果が格納されているので、回して取り出す。
		 	for(MsgMecabResult msg : result.resWordList)
		 	{
		 		System.out.println("単語:" + msg.name + " , 品詞:" + msg.pos + " , 品詞細分類1:" + msg.pos1);
		 	}
